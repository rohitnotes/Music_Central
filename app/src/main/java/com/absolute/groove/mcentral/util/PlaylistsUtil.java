...
import com.absolute.groove.mcentral.helper.M3UWriter;
import com.absolute.groove.mcentral.model.Playlist;
import com.absolute.groove.mcentral.model.PlaylistSong;
...
public class PlaylistsUtil {
    ...
    public static int createPlaylist(@NonNull final Context context, @Nullable final String absolute) {
        int id = -1;
        if (absolute != null && absolute.length() > 0) {
            try {
                Cursor cursor = context.getContentResolver()
                        .query(EXTERNAL_CONTENT_URI, new String[]{MediaStore.Audio.Playlists._ID},
                                MediaStore.Audio.PlaylistsColumns.NAME + "=?", new String[]{absolute}, null);
                if (cursor == null || cursor.getCount() < 1) {
                    final ContentValues values = new ContentValues(1);
                    values.put(MediaStore.Audio.PlaylistsColumns.NAME, absolute);
                    final Uri uri = context.getContentResolver().insert(EXTERNAL_CONTENT_URI, values);
                    if (uri != null) {
                        Toast.makeText(context, context.getResources().getString(R.string.created_playlist_x, absolute),
                                Toast.LENGTH_SHORT).show();
                        id = Integer.parseInt(uri.getLastPathSegment());
                    }
                } else {
